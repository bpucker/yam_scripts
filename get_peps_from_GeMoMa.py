...
__usage__ = """
					python get_peps_from_GeMoMa.py
...
def load_sequences( multiple_fasta_file ):
	...
	with open( multiple_fasta_file ) as f:
		...
		while line:
			if line[0] == '>':#bdvh
					sequences.update( { header: "".join( seq ).upper().replace( "Y", "N" ).replace( "R", "N" ).replace( "K", "N" ).replace( "S", "N" ).replace( "W", "N" ).replace( "M", "N" ).replace( "B", "N" ).replace( "D", "N" ).replace( "V", "N" ).replace( "H", "N" ) } )
				...
				if parts[2] == 'CDS':
					if not ";" in parts[-1]:
						...
						information.append( { 	'chr': parts[0],
																...
																'end': int( parts[4] ),
																'orientation': parts[6],
																'parent': parts[-1].split('arent=')[1].split(';')[0]
															} )
			line = f.readline()
